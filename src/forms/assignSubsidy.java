...
class assign_subsidy
{
	...
	JLabel namel,fatherl,genderl,mobilel,villagel,blockl,districtl,agel,farmer_id,vfarmer_id; 
	JTextField name,father,mobile,village,block,district,age;
	JRadioButton male=new JRadioButton("Male");
	JRadioButton female=new JRadioButton("Female");
	JFrame fr=new JFrame("Assign Project");
	JButton save;
	...
	JPanel pnl1;
	JTextField scost1;
	JComboBox scheme,progname;
	JLabel scostl1,schemel,prognamel;
	JLabel err1,err2,err3,err4,err5,err6,err7,err8,err9,err10,err11,err12,err13,err14;
	boolean chk1,chk2,chk3,chk4,chk5,chk6,chk7,chk8,chk9,chk10,chk11,chk12,chk13,chk14,chk15,chk16;
	JTextArea assist;
	JScrollPane scroll;
	Connection con;
	...
	String perc="^(100+|[0]+.[0]+|\\d{1,2}|\\.\\d{1,2}|\\d{1,2}\\.\\d{1,2})$";
	String decimal="[+]?\\d*\\.?\\d+";

	assign_subsidy(String user)
	{	u=user;
	fr.setLayout(null);
	...
	prognamel=new JLabel("Programme Name");
	prognamel.setOpaque(true);


	prognamel.setBounds(300, 100, 150, 20);
	fr.add(prognamel);

	progname=new JComboBox();
	progname.setBounds(450,100,100,20);

	//scheme.setEditable(true);
	fr.add(progname);

	progname.addItemListener(new ItemListener() {
	...
	progname.addFocusListener(new FocusListener() {

		@Override
		public void focusLost(FocusEvent arg0) {
			// TODO Auto-generated method stub
			if((progname.getSelectedItem()=="Select"))
			{
				chk16=false;
	...
	unit.addFocusListener(new FocusListener() {
		...
		public void focusLost(FocusEvent e) {
			...
			if(unit.getText().matches(regex3))
			{
				...
				err9.setText("Invalid Unit Count!");
				chk9=false;
			}
		}
	...
	void cost()
	{
		...
		try {
			pst=con.prepareStatement("select * from subsidy_govt where prog_name=? and scheme=?");
			pst.setObject(2, scheme.getSelectedItem());
			pst.setObject(1, progname.getSelectedItem());
			ResultSet rs=pst.executeQuery();
			if(rs.next())
	...
	{
		pst=con.prepareStatement("insert into subsidy_farmer (farmer_id,farmer_name,father_name,gender,mobile,village,block,district,age,scheme,program_name,total_cost,farmer_share,govt_share,assist,bank_name,account_no,ifsc) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
		...
		//------------------------------------------
		pst.setObject(11,progname.getSelectedItem());
		pst.setString(12,tcost.getText());
		pst.setString(13,scost.getText());
		pst.setString(14,scost1.getText());
		pst.setString(15,assist.getText());
		pst.setString(16,bank.getText());
		pst.setString(17,account.getText());
...
public class assignSubsidy {


	public static void main(String[] args) 
	{
		new assign_subsidy(null);
